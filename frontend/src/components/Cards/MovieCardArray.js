...
const MovieCardArray = ({ key_id, film }) => {
  const movieImg = `https://image.tmdb.org/t/p/original${film[3]}` || 'https://critics.io/img/movies/poster-placeholder.png';
      ...
      >
        <CardBody className='p-0 d-flex flex-column justify-content-end' style={{ background: `url(${movieImg})`, backgroundSize: 'cover', backgroundRepeat: 'no-repeat' }}>
          <div className='title-panel'>
            <h6 className='text-camelcase card-movie-title'>{film[1]}</h6>
          </div>
        </CardBody>
      </Card>
    </Link>
